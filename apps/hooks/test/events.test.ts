 ...
 *
 * The reader below is deliberately *not* `apps/web`'s decoder: this package
 * cannot import an app, and a test that shared the client's parser could not
 * catch the two of them agreeing on something wrong. It reads only what the
...
async function open(base: string, lastEventId?: string): Promise<Reader> {
